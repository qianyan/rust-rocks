        ...
        match sql {
            Statement::Query(q) => {
                ...
                let Select {
                    ...
                    _ => return Err(anyhow!("We only support Select Query at this moment")),
                };
            ...
            _ => Err(anyhow!("We only support Query at this moment")),
        }
    }
}
...
impl<'a> TryFrom<Source<'a>> for &'a str {
    ...
    fn try_from(source: Source<'a>) -> Result<Self, Self::Error> {
        if source.0.len() != 1 {
            return Err(anyhow!("We only support single data soruce at the moment"));
        }
...
mod tests {
    use super::*;
    use crate::PlatoDialect;
    use sqlparser::parser::Parser;

    #[test]
    fn parse_sql_should_work() {
